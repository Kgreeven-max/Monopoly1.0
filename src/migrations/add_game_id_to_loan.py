...
def run_migration():
    """Add game_id column to loans table"""
    logger.info("Starting migration to add game_id column to loans table")
    ...
    try:
        ...
        if not table_exists:
            logger.info("Table 'loans' doesn't exist yet, will be created with game_id column")
            return True
            
        # Check if the column already exists
        if check_column_exists('game_id', 'loans'):
            logger.info("Column game_id already exists in loans table; skipping migration")
            return True
        
        # Add game_id column
        query = text("""
            ALTER TABLE loans
            ADD COLUMN game_id VARCHAR(36)
        """)
        db.session.execute(query)
        
        # SQLite doesn't support adding constraints in ALTER TABLE
        # We'll need to add any constraints when creating a new loan
        
        db.session.commit()
        logger.info("Successfully added game_id column to loans table")
